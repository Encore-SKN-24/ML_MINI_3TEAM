...
* **[최하진]**: 머신러닝에 대한 이해가 아직 충분하지 않아 모델 성능을 개선하는 데 어려움이 있었지만, 모델을 직접 적용하고 결과를 비교해보는 과정에서 전처리, 특성 스케일링, 하이퍼파라미터 설정이 성능에 얼마나 큰 영향을 미치는지를 체감하며 모델의 구조와 동작 원리를 조금씩 이해하게 되었다. 다음에는 다른 모델들도 함께 비교해보고, 보다 정밀한 하이퍼파라미터 튜닝을 중심으로 성능 향상을 시도해보고 싶다.
